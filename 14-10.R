...
var_num_1<-c("Age","Balance","NetPromoterScore")
var_num_2<-c("Age","Balance","NetPromoterScore","DigitalEngagementScore")
...
ggplot(data, aes(x = IsActiveMember, y = Balance, fill = Exited)) +
  geom_boxplot() +
  labs(title = var, x = NULL, y = NULL) +
  theme_minimal() +
  theme(legend.position = "none",
...
x <- model.matrix(Exited ~ ., data = data[,-c(7,18)])[, -1]
