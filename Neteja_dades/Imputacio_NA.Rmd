...
data_for_imputation <- cbind(data_for_imputation, "ID" = data$ID, "Surname" = data$Surname, "Exited" = data$Exited)
...
```{r}
save(data_imputed_KNN, file = "C:/Users/Asus/Desktop/Quarto/Mineria_de_Datos/Treball_Mineria_de_Dades/Bases_de_dades/data_NA_imputed_KNN.RData")
```
...
```{r}
data_imputed_AREG_draw <- data_imputed_AREG |>
  select(!Surname)
data_draw <- data |>
  select(!Surname)
mass_before_after(data_draw, data_imputed_AREG_draw)
```
